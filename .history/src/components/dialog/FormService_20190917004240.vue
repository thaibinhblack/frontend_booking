    ...
    <v-dialog v-model="dialog" persistent max-width="600px">
        <form @submit.prevent="addService()">
            <v-card>
                ...
                <v-btn v-if="update == false" class="btn btn-add" text type="submit" ><v-icon>mdi-plus</v-icon> Thêm mới</v-btn>
                <v-btn v-else class="btn btn-add"text type="submit" ><v-icon>mdi-update</v-icon> Cập nhật</v-btn>
                </v-card-actions>
            </v-card>
            </form>
        </v-dialog>
