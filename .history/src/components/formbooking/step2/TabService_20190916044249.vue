    ...
    <v-list-item v-for="(service,i) in services" :key="i">
        <v-layout row class="item-service">
            <v-flex xs2 class="layout-check">
                <v-checkbox v-model="checkService" class="check-service" :value="service.UUID_SERVICE" @change="$emit('updateCheckService',checkService)"></v-checkbox>
            </v-flex>
            <v-flex xs10 class="image-service">
                <v-img :src="$store.state.PUBLIC_URL +service.IMAGE_SERVICE" />
            </v-flex>
        </v-layout>
    </v-list-item>
</v-list>
