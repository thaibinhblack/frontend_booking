...
props: ["tab", "id"],
  ...
  methods: {
    ...
    ApiGetDataRoom(uuid)
        {
            const date = new Date(this.getDateBooking)
            const result = date.getFullYear() + '-' + parseInt(date.getMonth() + 1) + '-' + date.getDate()
            console.log(result)
            this.$http.get(this.$store.state.API_URL + "booking/"+this.$store.BookingModule.state.booking.UUID_BOOKING+'?date='+result)
                .then((response) => {
                    const times = [
