...
export default {
    ...
    methods:
    {
        ...
        {
            this.$http.get(this.$store.API_URL + 'booking?phone='+this.$route.params.phone).then((response) => {
        ...
        NextStep()
        {
            ...
            else
            {
                ...
                data.append("DATE_BOOK",this.booking.DATE_BOOK.getFullYear()+'-'+ parseInt(this.booking.DATE_BOOK.getMonth() + 1 )+ '-' +this.booking.DATE_BOOK.getDate())
                data.append("TIME_BOOK",this.booking.TIME_BOOK)
                data.append("ACTION_BOOKING", 3)
                data.append("CHECK_BOOKING", 1)
                data.append("NOTE_BOOKING", 'Khách hàng đã đăng ký tới bước 3')
                if(this.booking.UUID_STYLIST != null)
                {
                     data.append("UUID_STYLIST",this.booking.UUID_STYLIST)
                }
                if(this.booking.CODE != null)
                {
