...
export default {
    props: ["step"],
    components:{
        'form-question': () => import('@/components/formbooking/stepdone/FormQuestion.vue')
    },
    watch:{
        step(newVal)
        {
    ...
    methods: 
    {
        ...
        ApiSMS()
        {
            this.$http.get(this.$store.state.API_URL + 'sms?phone='+parseInt(this.$route.params.phone))
        },
        ApiSendMail()
        {
            const data = new FormData()
