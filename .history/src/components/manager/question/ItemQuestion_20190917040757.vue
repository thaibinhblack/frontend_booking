<template>
     <v-row class="item-question" :class="{active: open}">
        ...
        <form-update-question-answer :dialog="dialog" :question="question_answer"/>
...
export default {
    components:{
        'form-update-question-answer': () => import('@/components/dialog/FormUpdateQuestion.vue')
    },
    ...
    methods:{
        ...
        updateQuestion(question)
        {
            ...
            this.dialog = true
            this.$http.get(this.$store.state.API_URL + 'answer?UUID_QUESTION='+question.UUID_QUESTION).then((response) => {
