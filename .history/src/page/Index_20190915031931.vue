...
export default {
    ...
    methods:{
        ...
        ApiCreateBooking()
        {
            ...
            this.$http.post(this.$store.state.API_URL + 'booking',data)
            .then((response) => {
                this.$router.push('/booking/'+this.booking.PHONE_BOOKING)
            }).catch((error) => {
                // console.log(error)
        ...
        check()
        {
            ...
            .then((response) => {
                ...
                {
                    this.message.type = 'warning'
                    this.message.text = 'Số điện thoại này chưa được đăng ký!'
                }
                else
                {
                    this.$router.push('/done/'+this.booking.PHONE_BOOKING)
                }
            })
        }
    },
