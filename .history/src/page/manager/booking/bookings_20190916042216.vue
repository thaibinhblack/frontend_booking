    ...
    <v-card>
        ...
        v-model="selected">
            ...
            <template v-slot:item.actions="{ item }">
                <v-icon class="btn-table" color="success" v-if="item.CHECK_BOOKING == 2" title="Xác nhận đăng ký" @click="checkBooking(item.UUID_BOOKING)">mdi-check</v-icon>
                <v-icon class="btn-table" color="warning" v-else title="Hủy đăng ký" @click="DeleteBooking(item.UUID_BOOKING)">mdi-check</v-icon>
...
export default {
    ...
    methods: {
        ApiGetBooking(){
            this.$http.get(this.$store.state.API_URL + 'booking')
            .then((response) => {
        ...
        {
            this.$http.put(this.$store.state.API_URL + "booking/"+UUID_BOOKING+"?check_booking=1")
            .then((response) => {
