    ...
    <v-card style="padding: 25px;">
        ...
        <v-row>
            ...
            <v-col cols="12" sm="12">
                ...
                <button disabled  class="btn btn-add active" v-else><v-icon>mdi-check</v-icon> Đã check</button>
            </v-col>
        </v-row>
    </v-card>
</v-col>
...
export default {
    ...
    methods:
    {
        ...
        SendSMS()
        {
            ...
            sms.append("phone", parseInt(this.booking.PHONE_BOOKING))
            sms.append("message", this.contentSMS)
            this.$http.post(this.$store.state.API_URL + 'sms?api_token='+this.$session.get('token'), sms).then((response) => {
                // console.log(response.data)
