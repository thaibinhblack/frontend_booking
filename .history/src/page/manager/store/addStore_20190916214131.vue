<template> 
    <div class="col-sm-12">
        <header-manager :title="'THÊM CHI NHÁNH MỚI'" :link="'Thêm mới chi nhánh'"  :store="true"/>
...
export default {
    ...
    methods:{
        ...
        addStore(){
            ...
            axios.post(this.BASE_URL + "store",this.store)
            .then((response) => {
                this.add = true
        ...
        FilterCountry()
        {
            axios.get('http://127.0.0.1:8000/api/v1/country/'+this.selectedProvince+'?type=UUID_PROVINCE')
            .then((response) => {
    ...
    components: {
        ...
        'message-alert': () => import('@/components/AlertMessage.vue'),
