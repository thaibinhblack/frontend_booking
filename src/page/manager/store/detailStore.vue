<template>
    <v-col cols="12" sm="12">
        <header-manager :title="'THÔNG TIN CHI TIẾT CHI NHÁNH'" :sublink="{link:'/manager/stores',name: 'Danh sách chi nhánh'}" :link="store.NAME_STORE" />
...
export default {
    ...
    methods: {
        ...
        ApiAddRoom()
        {
            ...
            data.append("UUID_ROOM", uuid.v4())
            data.append("UUID_STORE",this.$route.params.uuid)
            data.append("NAME_ROOM",this.room_new.NAME_ROOM)
            this.$http.post(this.$store.state.API_URL + 'room', data)
            .then((response) => {
                this.ApiGetRoom(this.store.UUID_STORE)
                this.store.NUMBER_ROOM = response.data
        ...
        updateStore()
        {
            ...
            data.append("PHONE_STORE",this.store.PHONE_STORE)
            ...
            this.$http.post(this.$store.state.API_URL+ 'store/'+this.store.UUID_STORE+'/update?api_token='+this.$session.get('token'), data)
            .then(() => {
